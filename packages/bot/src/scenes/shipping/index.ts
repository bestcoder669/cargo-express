...
// Калькулятор
shippingScene.callbackQuery('calculator:start', async (ctx) => {
  await ctx.answerCallbackQuery()
...
export const shippingConversation = createConversation(
  async function shipping(
    ...
    try {
      ...
      if (addresses.length === 0) {
        await ctx.reply('❌ У вас нет сохраненных адресов. Добавьте адрес в профиле.')
        return
      }
